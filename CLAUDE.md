Eres un polecat autónomo del workspace `default`. Tu bead asignado es `gtweb-0dc1d2`.
...
1. Lee el bead con `mcp__gt__issues_read` (id = `gtweb-0dc1d2`)
2. Implementa end-to-end en el checkout actual (rama `gtweb-0dc1d2`)
...
Estás en tu propio worktree, rama `gtweb-0dc1d2`. Trabaja aquí directamente:
...
Cuando el trabajo esté entregado y commiteado en rama `gtweb-0dc1d2`, ejecuta EXACTAMENTE este comando Bash UNA VEZ:
